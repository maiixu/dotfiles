...
version: 1.0.0
description: "Use when reviewing architecture or locking in a technical plan."
...
### Design Doc Check
```bash
SLUG=$(~/code/gstack/browse/bin/remote-slug 2>/dev/null || basename "$(git rev-parse --show-toplevel 2>/dev/null || pwd)")
...
```bash
~/code/gstack/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"issues_found":N,"mode":"MODE","commit":"COMMIT"}'
```
...
```bash
~/code/gstack/bin/gstack-review-read
```
