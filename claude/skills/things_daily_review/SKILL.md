...
Refer to `things_shared` for areas, routing guide, emoji conventions, and scheduling philosophy.
...
3. **Decide routing** — project (`list`), deadline, when — per scheduling philosophy in `things_shared`
...
Per writing protocol in `things_shared`. Skip items where action == "skip".
