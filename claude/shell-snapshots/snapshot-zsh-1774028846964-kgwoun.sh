...
kdo () {
	ps ax | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -i docker | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -E -iv 'grep|com.docker.vmnetd' | awk '{print $1}' | xargs kill
}
mkcd () {
	mkdir -p $@ && cd ${@:$#}
}
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  ...
  if [[ -n $ZSH_VERSION ]]; then
    ARGV0=rg /Users/maixu/.local/share/claude/versions/2.1.80 "$@"
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]] || [[ "$OSTYPE" == "win32" ]]; then
    ARGV0=rg /Users/maixu/.local/share/claude/versions/2.1.80 "$@"
  elif [[ $BASHPID != $$ ]]; then
    exec -a rg /Users/maixu/.local/share/claude/versions/2.1.80 "$@"
  else
    (exec -a rg /Users/maixu/.local/share/claude/versions/2.1.80 "$@")
  fi
}
fi
export PATH=/opt/homebrew/share/google-cloud-sdk/bin\:/opt/homebrew/Cellar/node/25.8.0/bin\:/Applications/Obsidian.app/Contents/MacOS\:/Users/maixu/.local/bin\:/usr/local/bin\:/System/Cryptexes/App/usr/bin\:/usr/bin\:/bin\:/usr/sbin\:/sbin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/local/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/bin\:/var/run/com.apple.security.cryptexd/codex.system/bootstrap/usr/appleinternal/bin\:/Library/Apple/usr/bin\:/Users/maixu/.cargo/bin\:/opt/homebrew/bin\:/opt/homebrew/sbin\:/Applications/iTerm.app/Contents/Resources/utilities\:/Users/maixu/.config/yt-dlp
