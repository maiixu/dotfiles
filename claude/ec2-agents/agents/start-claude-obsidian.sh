...
SESSION="claude-obsidian"
tmux kill-session -t "$SESSION" 2>/dev/null
sleep 1

tmux new-session -d -s "$SESSION" -x 220 -y 50 \
  -c "/Users/ec2-user/agents/obsidian" \
  "claude --dangerously-skip-permissions --dangerously-load-development-channels server:gchat-obsidian"
